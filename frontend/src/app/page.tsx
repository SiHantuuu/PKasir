...
export default function DashboardPage() {
  ...
  const handlePinSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const errorMessage =
        error instanceof Error ? error.message : 'Invalid PIN or NFC ID';
    ...
    try {
      ...
      setCardError(
        error instanceof Error
          ? error.message
          : 'Invalid card ID. Please try again.'
      );
      setNotification({
        isOpen: true,
        title: 'Scan Failed',
  ...
  const handlePay = async () => {
    ...
    if (scannedCard.balance < totalPrice) {
      setNotification({
        ...
        description: `Insufficient balance. You need ${formatCurrency(
          totalPrice - scannedCard.balance
        )} more.`,
        status: 'error',
      });
      return;
    ...
    try {
      ...
      const historyPromises = scannedProducts.map(async (product) => {
        ...
        console.log(
          `Processing payment for product: ${product.name} (ID: ${product.id})`
        );
        console.log(
          `Quantity: ${product.quantity}, Price per unit: ${product.price}, Total: ${productTotalPrice}`
        );
